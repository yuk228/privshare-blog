            ...
            <div className="px-[50px] py-[100px] md:px-[200px] lg:px-[500px]">
              ...
              <div className="flex justify-center">
                {post.frontMatter.tags?.map((tag) => (
                  <Link href={`/blogs/tags/${tag}`} className="border-2 hover:border-muted-foreground transition-all duration-300 rounded-md px-3 py-2 mr-2 mb-2" key={tag}>
                    {tag}
                  </Link>
                ))}
              </div>
              <ReactMarkdown
                ...
                components={{
                  ...
                  img: ({...props }) => <Image alt={post.frontMatter.title} src={props.src || ""} width={400} height={200} className="mx-auto" />,
              }}>
                {post.content}
              </ReactMarkdown>
            </div>
